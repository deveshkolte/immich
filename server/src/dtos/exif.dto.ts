...
export const ExifResponseSchema = z
  .object({
    ...
    exifImageWidth: z.int().min(0).nullish().describe('Image width in pixels'),
    exifImageHeight: z.int().min(0).nullish().describe('Image height in pixels'),
    fileSizeInByte: z.int().min(0).nullish().describe('File size in bytes'),
    ...
    iso: z.int().nullish().describe('ISO sensitivity'),
    ...
    rating: z.int().min(1).max(5).nullish().describe('Rating'),
  })
